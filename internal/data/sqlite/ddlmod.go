...
var (
	sqliteSeparator = "`|\"|'|\t"
	uniqueRegexp    = regexp.MustCompile(
		fmt.Sprintf(`^CONSTRAINT [%v]?[\w-]+[%v]? UNIQUE (.*)$`, sqliteSeparator, sqliteSeparator),
	)
	indexRegexp = regexp.MustCompile(
		fmt.Sprintf(
			`(?is)CREATE(?: UNIQUE)? INDEX [%v]?[\w\d-]+[%v]?(?s:.*?)ON (.*)$`,
			sqliteSeparator,
			sqliteSeparator,
		),
	)
		...
		if sections := tableRegexp.FindStringSubmatch(str); len(sections) > 0 {
			...
			for _, f := range result.fields {
				...
				if strings.HasPrefix(fUpper, "PRIMARY KEY") {
					...
					// data type length
					matches := regRealDataType.FindAllStringSubmatch(
						columnType.DataTypeValue.String, -1,
					)
...
func (d *ddl) renameTable(dst, src string) error {
	...
	if replaced == d.head {
		return fmt.Errorf(
			"failed to look up tablename `%s` from DDL head '%s'", src, d.head,
		)
	}
...
func (d *ddl) addConstraint(name string, constraintSQL string) {
	reg := regexp.MustCompile(
		"^CONSTRAINT [\"`]?" + regexp.QuoteMeta(name) + "[\"` ]",
	)

	for i := 0; i < len(d.fields); i++ {
		if reg.MatchString(d.fields[i]) {
			d.fields[i] = constraintSQL
			return
		}
	}

	d.fields = append(d.fields, constraintSQL)
}
